...
size_t p_registrationRequest(char *targetUrl, char *userid, char *appid, size_t mobiletype, char* mobileversion, char* mobilemodel, char* appversion, char **outData, size_t *outDataLen) {
	...
	if (!revChk && outDataTmp && outDataLenTmp > 0) {
		...
		*outDataLen = outDataLenTmp;
		...
		char* temErrMsg = p_getCommonErrMsg(OPERATION_REG);
...
/*
1-1. p_registrationRequestWithJson
...
size_t p_registrationRequestWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_registrationRequest(targetUrl, userid, appid, itjsmsg->mobiletype, itjsmsg->mobileversion, itjsmsg->mobilemodel, itjsmsg->appversion, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
...
/*
2-1. p_registrationResponseWithJson
...
size_t p_registrationResponseWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_registrationResponse(targetUrl, appid, sessionid, regresponsemsg, itjsmsg->authtype, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
...
size_t p_authenticationRequest(char *targetUrl, char *userid, char *appid, size_t mobiletype, char* mobileversion, char* mobilemodel, char* appversion, char **outData, size_t *outDataLen) {
	...
	if (!revChk && outDataTmp && outDataLenTmp > 0) {
		...
		*outDataLen = outDataLenTmp;
		...
		char* temErrMsg = p_getCommonErrMsg(OPERATION_AUTH);
		*outData = (char*)calloc(strlen(temErrMsg) + 1, sizeof(char));
		*outDataLen = strlen(temErrMsg);
		memcpy(*outData, temErrMsg, strlen(temErrMsg));
...
/*
3-1. p_authenticationRequestWithJson
...
size_t p_authenticationRequestWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		char *userid = itjsmsg->userid;
		char *appid = itjsmsg->appid;
		retVal = p_authenticationRequest(targetUrl, userid, appid, itjsmsg->mobiletype, itjsmsg->mobileversion, itjsmsg->mobilemodel, itjsmsg->appversion, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
...
/*
4-1. p_authenticationResponsWithJson
...
size_t p_authenticationResponseWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_authenticationResponse(targetUrl, appid, sessionid, authresponsemsg, itjsmsg->authtype, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
...
/*
5. p_transactionConfirmationRequest
...
/*
5-1. p_transactionConfirmationRequestWithJson
...
size_t p_transactionConfirmationRequestWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_transactionConfirmationRequest(targetUrl, userid, appid, contentType, contentEncodingType, content, itjsmsg->mobiletype, itjsmsg->mobileversion, itjsmsg->mobilemodel, itjsmsg->appversion, &outDataTmp, &outDataLenTmp);
...
size_t p_transactionConfirmationResponseWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_transactionConfirmationResponse(targetUrl, appid, sessionid, authresponsemsg, itjsmsg->authtype, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
...
/*
7-1. p_simpleAuthRequestWithJson
...
size_t p_simpleAuthRequestWithJson(char *targetUrl, char *jsmsg, char *b64nonid, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_simpleAuthRequest(targetUrl, userid, appid, b64pubkey, b64nonid, itjsmsg->mobiletype, itjsmsg->mobileversion, itjsmsg->mobilemodel, itjsmsg->appversion, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
...
/*
8-1. p_simpleAuthResponseWithJson
...
size_t p_simpleAuthResponseWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		retVal = p_simpleAuthResponse(targetUrl, appid, sessionid, authresponsemsg, itjsmsg->authtype, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
...
size_t p_deregistrationRequest(char *targetUrl, char *userid, char *appid, size_t mobiletype, char* mobileversion, char* mobilemodel, char* appversion, char **outData, size_t *outDataLen) {
	...
	if (!revChk && outDataTmp && outDataLenTmp > 0) {
		...
		*outDataLen = outDataLenTmp;
		...
		char* temErrMsg = p_getCommonErrMsg(OPERATION_DEREG);
		*outData = (char*)calloc(strlen(temErrMsg) + 1, sizeof(char));
		*outDataLen = strlen(temErrMsg);
		memcpy(*outData, temErrMsg, strlen(temErrMsg));
...
/*
9-1. p_deregistrationRequestWithJson
...
size_t p_deregistrationRequestWithJson(char *targetUrl, char *jsmsg, char **outData, size_t *outDataLen) {
	...
	if (itjsmsg) {
		...
		char *userid = itjsmsg->userid;
		char *appid = itjsmsg->appid;
		retVal = p_deregistrationRequest(targetUrl, userid, appid, itjsmsg->mobiletype, itjsmsg->mobileversion, itjsmsg->mobilemodel, itjsmsg->appversion, &outDataTmp, &outDataLenTmp);
		*outData = (char*)calloc(outDataLenTmp + 1, sizeof(char));
		*outDataLen = outDataLenTmp;
		memcpy(*outData, outDataTmp, outDataLenTmp);
		p_retDataFree(outDataTmp);
